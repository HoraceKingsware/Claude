...
⚠️ **注意**：此方式依赖搜狗微信搜索，**可能遇到反爬虫限制，不推荐使用**。
...
**⚠️ 重要提示**：此方式依赖搜狗微信搜索，可能会遇到反爬虫验证限制，成功率不稳定。**推荐使用方式一（从文章URL自动抓取）或方式四（批量URL抓取）**。
...
**Q: 提示"未获取到任何文章"或遇到反爬虫验证怎么办？**

A: 搜狗微信搜索有严格的反爬虫限制（antispider验证）。
...
2. **批量URL抓取**（最可靠）：
   - 手动收集文章URL（通过浏览器访问公众号历史消息）
   - 创建 `urls.txt` 文件，每行一个URL
   - 运行：`python scrape_batch.py --file urls.txt`

3. 如果搜狗搜索方式偶尔可用：
   - 等待一段时间后重试
   - 使用代理IP
   - 减少请求频率（降低 `--max-pages` 参数）
